  ...
  <div class="page-stack product-page">
    <section class="product-hero">
      <div>
        ...
        <h2>商品增删改查</h2>
        <p>维护商品名称、分类、售价、图片和销售状态；库存数量只在库存管理中调整。</p>
      </div>
    ...
    <section class="panel product-list-panel">
      <div class="product-toolbar">
        <el-input v-model.trim="filters.keyword" clearable placeholder="搜索商品名称" />
        ...
        <el-table-column label="商品" min-width="220">
          <template #default="{ row }">
            <div class="product-cell">
              ...
              <span v-else class="thumb">{{ row.name?.slice(0, 1) || '商' }}</span>
              <div>
                <strong>{{ row.name }}</strong>
                <small>{{ categoryName(categories, row.categoryId) || '未分类' }}</small>
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="110">
          <template #default="{ row }">
            <el-select
              ...
              @change="setProductStatus(row, $event)"
            >
              <el-option v-for="item in productStatusOptions" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
            <el-tag v-else :type="productStatusType(row.status)">{{ productStatusLabel(row.status) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="创建时间" min-width="160">
          <template #default="{ row }">{{ dateText(row.createdAt) }}</template>
        </el-table-column>
        <el-table-column v-if="canEditProduct" label="操作" width="150" fixed="right">
          <template #default="{ row }">
    ...
    <el-drawer v-model="drawerVisible" :title="editingProduct ? '编辑商品' : '新增商品'" size="420px">
      <el-form :model="form" class="product-drawer-form" label-position="top">
        <el-form-item label="商品名称">
          <el-input v-model.trim="form.name" maxlength="80" placeholder="例如 拿铁咖啡" />
        </el-form-item>
...
const activeCount = computed(() => products.value.filter(item => item.status === 1).length)
const disabledCount = computed(() => products.value.filter(item => item.status !== 1).length)
...
async function setProductStatus(product, status) {
  statusChangingId.value = product.id
  try {
    const updated = await productApi.setProductStatus(product.id, status)
    product.status = updated?.status ?? status
...
function productStatusLabel(status) {
  return productStatusOptions.find(item => item.value === status)?.label || '未知'
}
...
async function deleteProduct(product) {
  try {
    await ElMessageBox.confirm(`确认删除商品「${product.name}」？`, '删除商品', { type: 'warning' })
...
watch(() => filteredProducts.value.length, (total) => {
