<template>
  <div class="page-stack category-page">
    ...
    <el-dialog
      ...
      class="product-dialog category-dialog"
      align-center
      destroy-on-close
    >
      <div class="product-dialog-layout">
        <aside class="product-dialog-aside">
          <span class="dialog-kicker">分类档案</span>
          <strong>整理商品展示结构</strong>
          <p>排序值越小越靠前，停用分类会影响该分类下商品的销售状态。</p>
        </aside>
        <el-form
          :model="form"
          class="product-dialog-form category-dialog-form"
          label-position="top"
        >
          <el-form-item label="分类名称" class="span-2">
            <el-input
...
const enabledCount = computed(
  () => categories.value.filter((item) => item.status === 1).length,
);
...
async function setCategoryStatus(category) {
  ...
  try {
    ...
    ElMessage.success(
      nextStatus === 1 ? "分类已启用" : "分类已禁用，商品已停售",
    );
    await loadData();
  } catch (error) {
