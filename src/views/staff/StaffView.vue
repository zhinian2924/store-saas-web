  ...
  <div class="page-stack">
    <section class="panel">
      ...
      <el-table v-loading="loading" :data="staff" stripe empty-text="暂无员工">
        ...
        <el-table-column label="岗位" width="140">
          <template #default="{ row }">{{
            roleText[row.staffRole] || row.staffRole
          }}</template>
        </el-table-column>
          ...
          <template #default="{ row }">
            <div class="permission-tags">
              <el-tag
                ...
                :key="code"
                size="small"
                >{{ permissionText[code] || code }}</el-tag
              >
            </div>
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="form" label-position="top" class="staff-form">
        ...
        <el-form-item :label="editingId ? '新密码' : '登录密码'">
          <el-input
            ...
            show-password
            :placeholder="editingId ? '不修改可留空' : '请输入登录密码'"
          />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="saveStaff"
          >保存</el-button
        >
      </template>
    </el-dialog>
...
const permissionText = Object.fromEntries(
